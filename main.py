...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if 1 <= len(name) <= MAX_LENGTH:
        context.user_data['name'] = name
        await update.message.reply_text('لطفا نام خانوادگی خود را وارد کنید:')
        return LAST_NAME
    else:
        await update.message.reply_text(f'نام باید بین ۱ تا {MAX_LENGTH} حرف باشد. لطفا دوباره وارد کنید:')
        return NAME


async def get_last_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    last_name = update.message.text or "-"
    if len(last_name) <= MAX_LENGTH:
        context.user_data['last_name'] = last_name
        await update.message.reply_text('لطفا تخصص خود را وارد کنید:')
        return PROFESSION
    else:
        await update.message.reply_text(f'نام خانوادگی باید حداکثر {MAX_LENGTH} حرف باشد. لطفا دوباره وارد کنید:')
        return LAST_NAME
